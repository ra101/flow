...
const SidebarTrigger = ({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("m-1 size-10 rounded-2xl", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      {state=="expanded" ? <ArrowLeft />: <MenuIcon />}
...
const SidebarGroupLabel = ({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center justify-center rounded-md px-2 text-m font-medium outline-hidden opacity-65 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
